...
void SMcpStatusBarWidget::Construct(const FArguments& InArgs)
{
	...
	[
		...
		.ToolTipText(FText::FromString(TEXT("Open MUSE settings")))
		[
			SNew(SHorizontalBox)
			+ SHorizontalBox::Slot()
			.AutoWidth()
...
{
	if (!GEditor) return FText::FromString(TEXT("MUSE"));
	...
	if (Subsystem && Subsystem->NativeTransport && Subsystem->NativeTransport->IsRunning())
	{
		...
		return FText::FromString(FString::Printf(TEXT("MUSE :%d (%d)"), Port, Sessions));
	}

	return FText::FromString(TEXT("MUSE off"));
}
